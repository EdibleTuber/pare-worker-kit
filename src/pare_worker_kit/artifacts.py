...
def artifact_path(root: str, slug: str, name: str) -> str:
    ...
    *persistent* redirection, not a racing one. sha256-after-transfer, above,
    is the control that survives an attacker who wins the race.
    ...
    # Belt and braces. Unreachable while the two regexes above forbid a
    # separator and the root is normalised -- which is the point: it is here
    # so that a future edit loosening either one cannot quietly reopen
    # traversal without a test noticing.
